...
//获取地图在线玩家语句
//普通方式
$sql = "SELECT `gt_name` FROM `game_user` WHERE `pk.map_pet.id` = {$o_pet_id} AND `is_online` =1 AND `user_id` != {$user_id} LIMIT 0, 5";
//开始获取
if ($sql) {
    $result = sql($sql);
    while (list($gt_name) = $result->fetch_row()) {
        $ocount++;
        if ($ocount == 1) {
            $user_arr .= '参战' . $ocount .'：' . $gt_name . '<br>';
        } else {
            $user_arr = $user_arr . '参战' . $ocount .'：' . $gt_name . '<br>';
        }
    }
}
//战斗状况
